...
const CSV_HEADER = "timestamp,id,url\n";
...
const appendCsvLine = async (filePath: string, id: string, url: string): Promise<void> => {
  const timestamp = new Date().toISOString();
  await appendFile(filePath, `${timestamp},${id},${url}\n`, "utf8");
};
...
export class ProgressTracker {
  ...
  async loadFinishedIds(): Promise<Set<string>> {
    return readCsvAsSet(this.finishedPath);
  }

  async markInProgress(summary: FacilitySummary): Promise<void> {
    await appendCsvLine(this.inProgressPath, summary.id, summary.profileUrl);
  }

  async markFinished(summary: FacilitySummary): Promise<void> {
    await appendCsvLine(this.finishedPath, summary.id, summary.profileUrl);
  }
}
