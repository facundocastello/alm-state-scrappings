...
import { readInputCsv, writeFacilitiesCsv } from "./csv.js";
...
const run = async (): Promise<void> => {
  ...
  const summaries = await readInputCsv();
  const pendingSummaries = summaries.filter((summary) => !finishedFids.has(summary.fid));

  console.log(
    `Found ${summaries.length} facilities in input CSV, ${pendingSummaries.length} pending (skipped ${summaries.length - pendingSummaries.length} already finished).`
  );
