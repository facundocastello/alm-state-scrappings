...
import { expectedNJ60104, compareResults } from './testExpected.js';
import { FacilityRecord } from './types.js';

async function test() {
  const testId = 'NJ60104'; // Complete Care At Linwood - nursing home with full data
  ...
  try {
    ...
    console.log(`  Scope & Severity: ${facilityDetail.inspectionSummaries.routine.maxScopeAndSeverity || '(none)'}`);
    ...
    console.log(`  Scope & Severity: ${facilityDetail.inspectionSummaries.complaint.maxScopeAndSeverity || '(none)'}`);
    ...
    const actualRecord: FacilityRecord = {
      ...facilityDetail,
      reports,
    ...
    compareResults(actualRecord, expectedNJ60104);
